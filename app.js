...
function render() {
    ...
    if (dir == 'LEFT') {
        snakeX = snakeX - snakeBox
    }
    if (dir == 'RIGHT') {
        snakeX = snakeX + snakeBox
    }
    if (dir == 'UP') {
        snakeY = snakeY - snakeBox
    }
    if (dir == 'DOWN') {
        snakeY = snakeY + snakeBox
    }

    // snake gets food->getFood()
    ...
    snake.unshift(newHead) //unshift function adds an element in the first position of an array
...
function gameOver() {
    ...
    dead.play()
    ctx.fillStyle = 'red'
    ctx.font = '30px tahona'
    ctx.fillText('Game Over!', canvasSize / 2 - 100, canvasSize / 2)
    ctx.fillStyle = 'black'
    ctx.fillText(`Your Score: ${score}`, canvasSize / 2 - 100, canvasSize / 2 + 40)
}